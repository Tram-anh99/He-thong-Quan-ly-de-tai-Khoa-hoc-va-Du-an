...
async function main() {
  ...
  const project = await prisma.project.upsert({
    ...
    create: {
      ...
      fullText: '<h2>1. Đặt vấn đề</h2><p>Trong bối cảnh số hóa mạnh mẽ, việc quản lý tài liệu khoa học cần được hiện đại hóa...</p>',
      ownerId: pi.id,
  ...
  await prisma.projectMember.createMany({
    data: [
      { projectId: project.id, userId: pi.id, roleInProject: 'Chủ nhiệm', allocation: 40 },
      { projectId: project.id, userId: researcher.id, roleInProject: 'Thành viên', allocation: 30 },
    ],
    skipDuplicates: true,
  });
    ...
    data: [
      {
        ...
        description: 'Khảo sát các công trình liên quan trong và ngoài nước',
        amount: 50000000,
        assignedToId: pi.id,
        status: 'completed',
        sortOrder: 1,
    ...
    data: [
      {
        ...
        description: 'Biểu mẫu hợp đồng khoán giao việc chuyên môn cho cá nhân',
        category: 'hop_dong',
        fileUrl: '/templates/hop-dong-khoan-chuyen-mon.docx',
        placeholders: ['{{Ten_Du_An}}', '{{Nguoi_Thuc_Hien}}', '{{Chuc_Vu}}', '{{Kinh_Phi_Bang_So}}', '{{Kinh_Phi_Bang_Chu}}', '{{Ngay_Bat_Dau}}', '{{Ngay_Ket_Thuc}}', '{{Noi_Dung_Cong_Viec}}'],
      },
      {
        ...
        placeholders: ['{{Ten_Du_An}}', '{{Ten_San_Pham}}', '{{Nguoi_Thuc_Hien}}', '{{Ngay_Nghiem_Thu}}', '{{Ket_Luan}}'],
      },
      {
        ...
        placeholders: ['{{So_Hop_Dong}}', '{{Ten_Du_An}}', '{{Nguoi_Thuc_Hien}}', '{{Tong_Kinh_Phi}}', '{{Ngay_Thanh_Ly}}'],
      },
      {
        name: 'Đề nghị tạm ứng',
        description: 'Biểu mẫu đề nghị tạm ứng kinh phí',
        category: 'de_nghi_tam_ung',
        fileUrl: '/templates/de-nghi-tam-ung.docx',
        placeholders: ['{{Ten_Du_An}}', '{{Nguoi_De_Nghi}}', '{{So_Tien_Bang_So}}', '{{So_Tien_Bang_Chu}}', '{{Ly_Do}}', '{{Ngay_De_Nghi}}'],
      },
      {
        name: 'Bảng kê thanh toán',
        description: 'Bảng kê chi tiết thanh toán kinh phí',
        category: 'bang_ke',
        fileUrl: '/templates/bang-ke-thanh-toan.docx',
        placeholders: ['{{Ten_Du_An}}', '{{Danh_Sach_Hang_Muc}}', '{{Tong_Tien_Bang_So}}', '{{Tong_Tien_Bang_Chu}}', '{{Ky_Thanh_Toan}}'],
      },
    ],
    skipDuplicates: true,
  });
