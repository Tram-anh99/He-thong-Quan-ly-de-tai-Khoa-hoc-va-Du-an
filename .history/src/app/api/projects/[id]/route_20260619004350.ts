...
export async function GET(request: NextRequest, { params }: Params) {
  try {
    ...
    const project = await prisma.project.findUnique({
      ...
      include: {
        owner: {
          select: { id: true, fullName: true, position: true, department: true, email: true },
        },
        members: {
          include: {
            user: {
        ...
        paymentRecords: {
          include: {
            ...
            contract: { select: { id: true, title: true, contractNo: true } },
          },
          orderBy: { createdAt: 'desc' },
        },
      },
...
export async function PUT(request: NextRequest, { params }: Params) {
  try {
    ...
    const canEdit = ['ADMIN', 'MANAGER'].includes(user.role) || existing.ownerId === user.id;
    if (!canEdit) return errorResponse('Bạn không có quyền sửa dự án này', 403);
    ...
    if (code && code !== existing.code) {
      const codeExists = await prisma.project.findUnique({ where: { code } });
      if (codeExists) return errorResponse(`Mã đề tài "${code}" đã tồn tại`);
    }
