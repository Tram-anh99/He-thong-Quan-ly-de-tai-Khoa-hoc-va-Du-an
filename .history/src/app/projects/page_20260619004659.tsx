...
import {
  Table, Button, Input, Select, Tag, Space, Card, Typography, Row, Col,
  Modal, Form, InputNumber, DatePicker, message, Tooltip, Popconfirm, Empty,
} from 'antd';
import {
...
interface Project {
  ...
  year: number;
  status: string;
  owner: { id: string; fullName: string; position: string | null; department: string | null };
  _count: { members: number; budgetItems: number; contracts: number; products: number };
}
...
export default function ProjectsPage() {
  ...
  const handleCreate = async (values: any) => {
    try {
      ...
      message.error(err.response?.data?.error || 'Không thể tạo dự án');
    }
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      message.error(err.response?.data?.error || 'Không thể xóa dự án');
    }
  };
  ...
  const columns = [
    {
      ...
      key: 'code',
      width: 130,
      render: (code: string | null) => code ? <Text strong>{code}</Text> : <Text type="secondary">—</Text>,
    },
    {
      ...
      render: (title: string, record: Project) => (
        <a onClick={() => router.push(`/projects/${record.id}`)}>{title}</a>
      ),
    },
    {
      title: 'Chủ nhiệm',
      ...
      render: (_: any, record: Project) => (
        <Space size="small">
          <Tooltip title="Xem chi tiết">
            <Button type="text" size="small" icon={<EyeOutlined />} onClick={() => router.push(`/projects/${record.id}`)} />
          </Tooltip>
  ...
  return (
    <AppLayout>
      <div style={{ marginBottom: 24 }}>
        ...
        <Text type="secondary">Quản lý tất cả đề tài nghiên cứu khoa học và dự án</Text>
      </div>
      ...
      >
        ...
        >
          <Row gutter={16}>
            ...
            <Col span={8}>
              <Form.Item label="Năm thực hiện" name="year" rules={[{ required: true, message: 'Bắt buộc' }]}>
          ...
          <Form.Item label="Tóm tắt" name="summary">
            <TextArea rows={2} placeholder="Mô tả ngắn gọn về đề tài" />
          </Form.Item>

          <Row gutter={16}>
            <Col span={8}>
              <Form.Item label="Tổng kinh phí (VNĐ)" name="totalBudget">
                <InputNumber
                  style={{ width: '100%' }}
                  min={0}
                  step={1000000}
            ...
            <Col span={16}>
              <Form.Item label="Nguồn kinh phí" name="fundingSource">
                <Select allowClear placeholder="Chọn nguồn kinh phí">
          ...
          <Form.Item style={{ textAlign: 'right', marginBottom: 0 }}>
            <Space>
              ...
              <Button type="primary" htmlType="submit">Tạo dự án</Button>
            </Space>
          </Form.Item>
        </Form>
      </Modal>
