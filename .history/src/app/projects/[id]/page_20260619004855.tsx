...
import {
  Card, Tabs, Descriptions, Tag, Typography, Space, Button, Table, Empty,
  message, Spin, Divider, Row, Col, Statistic, List, Upload, Modal, Form,
  Input, Select, InputNumber, Popconfirm, Badge, Tooltip,
} from 'antd';
import {
...
export default function ProjectDetailPage() {
  ...
  // Budget summary
  const totalBudgetPlanned = project.budgetItems?.reduce(
    (sum: number, item: any) => sum + Number(item.amount || 0), 0
  ) || 0;
  const totalBudgetPaid = project.paymentRecords?.reduce(
    (sum: number, r: any) => r.status === 'COMPLETED' ? sum + Number(r.amount || 0) : sum, 0
  ) || 0;

  const tabItems = [
    // Tab 1: Thong tin & Thuyet minh
    {
      key: 'info',
      label: <span><FileTextOutlined /> Thông tin & Thuyết minh</span>,
      children: (
        <Card>
          <Descriptions bordered column={{ xxl: 3, xl: 3, lg: 2, md: 2, sm: 1, xs: 1 }} size="middle">
            <Descriptions.Item label="Mã đề tài">
              <Text strong copyable>{project.code || '—'}</Text>
            </Descriptions.Item>
            ...
            <Descriptions.Item label="Chủ nhiệm">
              <Space>
                ...
                {project.owner?.position && <Text type="secondary">({project.owner.position})</Text>}
              </Space>
            </Descriptions.Item>
            <Descriptions.Item label="Bộ môn / Phòng ban">
              {project.owner?.department || '—'}
            </Descriptions.Item>
            <Descriptions.Item label="Năm thực hiện">
              <Tag color="blue">{project.year}</Tag>
            </Descriptions.Item>
            <Descriptions.Item label="Tổng kinh phí">
              <Text strong style={{ color: '#cf1322' }}>{formatCurrency(project.totalBudget)}</Text>
            </Descriptions.Item>
            <Descriptions.Item label="Bằng chữ" span={2}>
              <Text italic>{amountToWords(project.totalBudget)}</Text>
            </Descriptions.Item>
    ...
    // Tab 2: Nhan su
    {
      ...
      children: (
        <Card
          ...
          extra={<Button type="primary" size="small" icon={<PlusOutlined />}>Thêm thành viên</Button>}
        >
          <Table
            ...
            columns={[
              ...
              {
                title: 'Vai trò', dataIndex: 'roleInProject', key: 'roleInProject',
                render: (role: string) => <Tag color={role === 'Chủ nhiệm' ? 'blue' : 'default'}>{role || '—'}</Tag>,
              },
              {
                title: 'Tỷ lệ (%)', dataIndex: 'allocation', key: 'allocation', align: 'center' as const,
    ...
    // Tab 3: Ngan sach & Du toan
    {
      key: 'budget',
      label: <span><DollarOutlined /> Ngân sách & Dự toán</span>,
      children: (
        <>
          <Row gutter={16} style={{ marginBottom: 16 }}>
            <Col span={8}>
              <Card>
                <Statistic
                  ...
                  formatter={(v) => formatVND(v as number) + ' VNĐ'}
            ...
            <Col span={8}>
              <Card>
                <Statistic
                  title="Kinh phí dự toán (theo hạng mục)"
                  value={totalBudgetPlanned}
                  formatter={(v) => formatVND(v as number) + ' VNĐ'}
          ...
          <Card
            title="Hạng mục dự toán"
            extra={
              <Space>
                <Upload showUploadList={false} accept=".xlsx,.xls,.pdf">
                  <Button icon={<UploadOutlined />}>Upload file dự toán</Button>
                </Upload>
                <Button type="primary" size="small" icon={<PlusOutlined />}>Thêm hạng mục</Button>
              </Space>
            }
          >
            <Table
              dataSource={project.budgetItems || []}
              rowKey="id"
              size="middle"
              pagination={false}
              summary={() => (
                <Table.Summary.Row>
                  ...
                  <Table.Summary.Cell index={3} align="right">
                    <Text strong style={{ color: '#cf1322' }}>{formatCurrency(totalBudgetPlanned)}</Text>
                  </Table.Summary.Cell>
              ...
              columns={[
                ...
                {
                  title: 'Người thực hiện', dataIndex: ['assignedTo', 'fullName'], key: 'assignedTo',
                  render: (name: string) => name || <Text type="secondary">Chưa phân công</Text>,
                },
                {
                ...
                },
                {
                  ...
                  render: () => (
                    <Space size="small">
                      <Tooltip title="Tạo hồ sơ thanh toán"><Button type="text" size="small" icon={<FileWordOutlined />} /></Tooltip>
    ...
    // Tab 4: Quyet dinh & San pham
    {
      key: 'products',
      label: <span><FileTextOutlined /> Quyết định & Sản phẩm</span>,
      children: (
        <Row gutter={16}>
          <Col span={12}>
            <Card
              title="📁 Tài liệu đính kèm"
              extra={<Upload showUploadList={false}><Button icon={<UploadOutlined />} size="small">Upload</Button></Upload>}
            >
              {project.documents?.length > 0 ? (
                <List
                  ...
                  renderItem={(doc: any) => (
                    <List.Item
                      actions={[<Button key="dl" type="link" size="small" icon={<DownloadOutlined />}>Tải</Button>]}
                    >
                      <List.Item.Meta
                        avatar={doc.fileType?.includes('pdf') ? <FilePdfOutlined style={{ fontSize: 24, color: '#ff4d4f' }} /> : <FileWordOutlined style={{ fontSize: 24, color: '#1677ff' }} />}
                        title={doc.name}
                        description={<Space><Text type="secondary">{doc.category || 'Tài liệu'}</Text><Text type="secondary">• {formatDate(doc.createdAt)}</Text></Space>}
                      />
                    </List.Item>
                  )}
                />
          ...
          <Col span={12}>
            <Card
              title="📦 Sản phẩm giao nộp"
              extra={<Button type="primary" size="small" icon={<PlusOutlined />}>Thêm sản phẩm</Button>}
            >
              {project.products?.length > 0 ? (
                <List
                  ...
                  renderItem={(product: any) => (
                    <List.Item
                      actions={product.fileUrl ? [<Button key="dl" type="link" size="small" icon={<DownloadOutlined />}>Tải</Button>] : []}
                    >
                      <List.Item.Meta
                        avatar={product.fileType === 'code' ? <CodeOutlined style={{ fontSize: 24, color: '#722ed1' }} /> : product.fileType === 'pdf' ? <FilePdfOutlined style={{ fontSize: 24, color: '#ff4d4f' }} /> : <FileWordOutlined style={{ fontSize: 24, color: '#1677ff' }} />}
                        title={product.title}
                        description={
                          <Space direction="vertical" size={0}>
                            <Text type="secondary">{product.description || '—'}</Text>
                            <Text type="secondary" style={{ fontSize: 12 }}>Giao nộp: {formatDate(product.deliverDate)}</Text>
                          </Space>
                        }
                      />
                    </List.Item>
                  ...
                  columns={[
                    ...
                    { title: 'Kinh phí', dataIndex: 'amount', width: 150, align: 'right' as const, render: (v: string) => formatVND(v) + ' VNĐ' },
                    {
                      ...
                      render: (s: string) => {
                        ...
                        const st = map[s] || { color: 'default', label: s };
  ...
  return (
    <AppLayout>
      <div style={{ marginBottom: 16 }}>
        <Button type="text" icon={<ArrowLeftOutlined />} onClick={() => router.push('/projects')} style={{ marginBottom: 8 }}>
          Quay lại danh sách
        </Button>
